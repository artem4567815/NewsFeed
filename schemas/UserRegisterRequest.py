...
class UserRegisterRequest(BaseModel):
    ...
    corpus: str = Field(..., min_length=1, max_length=100)
    ...
    def validate_password(cls, value):
        ...
        if not (8 <= len(value) <= 60 and any(c.isupper() for c in value) and any(c.islower() for c in value) and any(
                c.isdigit() for c in value) and any(c in special_characters for c in value) and all(
            c in allowed_characters for c in value)):
